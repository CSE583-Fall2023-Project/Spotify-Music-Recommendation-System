...
<p> 
The scope of this project emcompasses two major aspects.  First, we hope to **inform the historical music insights**, uncovering the story of music behind the data.  Leveraging the Spotify music dataset, we aim to discover and illustrate interesting patterns and trends through interactive visualization dashboards with customizable controls.   Second, we want to **provide personalized recommendations** based on user behavioral preferences and potentially social networks.  Through understanding then predicting the unique tastes of each user, we hope to improve users’ listening experience.  Ultimately, we want to create a platform where users can explore the rich history of contemporary music and discover new personal favorites, all within an engaging interactive user-friendly interface.  We aim to create a music discovery experience that is not only enjoyable but also insightful and personal.    
</p>
